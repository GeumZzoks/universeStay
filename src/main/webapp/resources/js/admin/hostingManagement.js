                ...
                if (error) {
                    ...
                    roomList.forEach(room_id => {
                        const status_id = document.querySelector(
                                "td[value='" + room_id + "']");
                        status_id.innerHTML = (status === "승인 완료") ? '승인 완료'
                                : '승인 반려';
...
searchBtn.addEventListener('click', function () {
    ...
    .then(data => {
        ...
        for (let i = 0; i < data.length; i++) {
            const room = data[i];
            room.created_at = new Date(room.created_at).getFullYear() + '-'
                    + ('0'
                            + (new Date(room.created_at).getMonth() + 1)).slice(
                            -2) + '-' + ('0'
                            + (new Date(room.created_at).getDate())).slice(-2)
                    + ' ' + ('0'
                            + (new Date(room.created_at).getHours())).slice(-2)
                    + ':' + ('0'
                            + (new Date(room.created_at).getMinutes())).slice(
                            -2);
