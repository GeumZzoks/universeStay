...
public class InquiryDto {
    ...
    @ApiModelProperty(notes = "내용", example = "호스트와 소통하는데 답변이 느려서 일정을 계획하는데 문제가 있습니다. 해당 호스트의 예약을 철회하고 환불받고 싶습니다.", required = false)
    private String inquiry_ctt;

    @ApiModelProperty(notes = "작성일", example = "YYYY-MM-DD hh:mm:ss.000000", required = false)
    private Timestamp inquiry_created_at;

    @ApiModelProperty(notes = "수정일", example = "YYYY-MM-DD hh:mm:ss.000000", required = false)
    private Timestamp inquiry_updated_at;

    @ApiModelProperty(notes = "조회수", example = "N", required = false)
    private Integer inquiry_hit;

    @ApiModelProperty(notes = "공개 여부", example = "N", required = true)
    private String pinquiry_is_open;
