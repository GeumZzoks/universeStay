...
public class UserLoginController2 {
    ...
    public String login(String user_email, String user_pwd, String remember_id,
        ...
        System.out.println("statusId 컨트롤러다= " + statusId);

        System.out.println("USER INFO == "+userInfo);
        try {
            if (userInfo != null) { // 3. 로그인 시도 시
                System.out.println("statusId ddddddddddddddd= " + statusId);

                // 회원탈퇴한 유저가 로그인 시도 시 메인으로 이동 후 알럿창 띄우기.
                if(statusId.equals("U04")) {
//                    model.addAttribute("statusId", statusId);
                    System.out.println("statusId 타니타니= " + statusId);
                    redirectAttributes.addFlashAttribute("statusId", "U04");
                    System.out.println("sfdddffdfdf");
...
//            return null; // => 리턴 사용 예) 우리가 정의한 에러페이지가 있다면 보여주면 되는데, 없으니까 안 사용
    ...
    //일단 이게 아닌거 같은데(코드가 중복됨)ㅜㅜㅜㅜ  따로 뻈음.........
//    @PostMapping("/Status")
//    public String userStatus(String user_email, String user_pwd,Model model) {
//        UserDto userInfo = userLoginService.signin(user_email, user_pwd, session); // 이 호출은 세션에 저장도 하는데.. 맞는지 모르겠음
//
//        }
//
//    }
